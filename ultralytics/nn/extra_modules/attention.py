...
__all__ = ['EMA', 'SimAM', 'SpatialGroupEnhance', 'BiLevelRoutingAttention', 'BiLevelRoutingAttention_nchw', 'TripletAttention', 
           ...
           'LocalWindowAttention','CrossDomainSharedAttention','Coordinateawareattention','CBAM','CA_Block']
...
class CBAM(nn.Module):
    ...
    def __init__(self, c1, c2, ratio=16, kernel_size=7):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.channel_attention = ChannelAttention_CBAM(c1, ratio)
        self.spatial_attention = SpatialAttention_CBAM(kernel_size)
...
class ECANet(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.act1(output)

class CrossDomainSharedAttention(nn.Module):
    def __init__(self, channels):
        super(CrossDomainSharedAttention, self).__init__()
        ...
        # )
        #空间域注意力图生成
        self.spatial_attention=SpatialModule()
        ...
        # )
        # 通道域注意力图生成
        self.channel_attention=ECANet(channels // 2)
        
        
        
        # 1x1卷积用于调整通道数
        self.channel_adjust = nn.Conv2d(channels, channels, 1, bias=False)
    ...
    def forward(self, x):
        ...
        # 生成空间域注意力图并应用
        spatial_att_map = self.spatial_attention(shared_features)
        spatial_att = spatial_att_map * x1
        
        # 生成通道域注意力图并应用
        channel_att_map = self.channel_attention(shared_features)
        channel_att = channel_att_map * x2
        
        # 拼接空间和通道注意力处理后的特征图
        merged = torch.cat([spatial_att, channel_att], dim=1)
        
        # 通过1x1卷积调整通道数
        result = self.channel_adjust(merged)
...
class Coordinateawareattention(nn.Module):
    ...
    def __init__(self, channels) -> None:
        ...
        self.pool_h = nn.AdaptiveAvgPool2d((None, 1))
        self.pool_w = nn.AdaptiveAvgPool2d((1, None))
        self.conv_hw = Conv(channels//4, channels//4, (3, 3))
        # self.conv_h = Conv(channels, channels, (1, 3))
        # self.conv_w = Conv(channels, channels, (3, 1))
        self.conv_pool_hw = Conv(channels//4, channels//4, 1)
        self.conv_four = Conv(channels//4, channels//4, 1)
        self.final=Conv(channels,channels,1)
    def forward(self, x):
        _, _, h, w = x.size()
        x_split = torch.chunk(x, 4, dim=1)  # Split the input tensor into 4 parts along the channel dimension
        # print(x.shape)
        # print(x_split[0].shape)
        x_pool_h, x_pool_w, x_pool_ch = self.pool_h(x_split[0]), self.pool_w(x_split[1]).permute(0, 1, 3, 2), self.gap(x_split[2])
        x_pool_hw = torch.cat([x_pool_h, x_pool_w], dim=2)
        x_pool_hw = self.conv_hw(x_pool_hw)
        x_pool_h, x_pool_w = torch.split(x_pool_hw, [h, w], dim=2)
        x_pool_hw_weight = self.conv_pool_hw(x_pool_hw).sigmoid()
        x_pool_h_weight, x_pool_w_weight = torch.split(x_pool_hw_weight, [h, w], dim=2)
        x_pool_h, x_pool_w = x_pool_h * x_pool_h_weight, x_pool_w * x_pool_w_weight
        # print(x_pool_ch.shape,torch.mean(x_pool_hw_weight, dim=2, keepdim=True).shape)
        x_pool_ch = x_pool_ch * torch.mean(x_pool_hw_weight, dim=2, keepdim=True)
        # print(out.shape)
        
        return self.final(torch.cat([x_split[0] * x_pool_h.sigmoid() ,x_split[1]*x_pool_w.permute(0, 1, 3, 2).sigmoid() , x_split[2]*x_pool_ch.sigmoid(),self.conv_four(x_split[3])],dim=1))