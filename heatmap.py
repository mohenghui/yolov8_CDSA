...
class yolov8_heatmap:
    ...
    def process(self, img_path, save_path):
        # img process
        orig_img = cv2.imread(img_path)  # 保留原始图像尺寸
        img, _, _ = letterbox(orig_img.copy(), new_shape=(640, 640))
        ...
        # 调整热力图尺寸回原始图像尺寸
        cam_image_resized = cv2.resize(cam_image, (orig_img.shape[1], orig_img.shape[0]))
        cam_image_resized = Image.fromarray(cam_image_resized)
        cam_image_resized.save(save_path)
...
def get_params():
    params = {
        'weight': '/root/autodl-tmp/ultralytics-main/runs/detect/train8/weights/best.pt', # 现在只需要指定权重即可,不需要指定cfg
        'device': 'cuda:0',
...
if __name__ == '__main__':
    ...
    model(r'/root/autodl-tmp/ultralytics-main/datasets/mydata_duoye_all/images', 'cam_result')